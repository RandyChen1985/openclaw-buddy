...
- [ ] **Hook 稳定性校验**: 验证 `useTaskCenter` 在任务状态更新及 API 轮询时，不应触发 React 无限重渲染循环，控制台无重复 API 报错。
- [ ] **CPU 负载监控**: 页面静默状态下，浏览器 CPU 占用应处于极低水平（<1%）。
- [ ] **通知风暴防护**: 验证单一任务状态变更仅触发一次 `notification` 弹窗，无重复堆叠。
