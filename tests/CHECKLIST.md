...
- [ ] **菜单可见性**: “专家市场”后应出现“安全中心”菜单项，且图标为 `ShieldCheck`。
- [ ] **策略卡片展示**: 页面应能正确展示当前生效的 `ask` 和 `security` 状态。
- [ ] **预设切换任务化**: 点击 `YOLO`, `Cautious` 或 `Deny All` 按钮后，应立即在右下角弹出异步任务通知。
- [ ] **策略更新验证**: 切换预设任务完成后，再次点击“刷新”或观察卡片，状态应同步更新。
- [ ] **Agent 选择器**: 白名单区域的下拉框应包含当前系统中所有已安装的机器人。
- [ ] **白名单实时同步**: 为特定 Agent 添加/删除指令模式后，表格应在任务完成后自动刷新。
- [ ] **JSON 模式编辑**: 弹出 JSON 编辑器后，修改内容并保存，应触发 `set-approvals` 异步任务且生效。
- [ ] **任务中心联动**: 安全中心的所有写操作（Preset/Allowlist）均应在“任务中心”历史记录中可查。
