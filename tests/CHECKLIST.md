...
- [ ] **串行任务调度**: 连续提交多个机器人修改任务，验证其在后端是否按顺序“排队”执行，而非并发触发。
- [ ] **优先级插队**: 在普通机器人任务执行期间，提交“重启网关”任务，验证高优先级任务能否优先被调度器消费。
- [ ] **任务中心自动同步**: 在不手动刷新的情况下，验证任务面板能否在 8 秒内自动从后端同步任务状态。
- [ ] **自愈避让机制**: 当用户正在手动排队操作网关时，验证 Guardian 日志中是否出现 “Module gateway is busy, skipping heal” 的物理避让记录。
