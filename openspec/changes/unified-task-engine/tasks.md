...
- [ ] 5.1 创建 `web/src/hooks/useTaskCenter.ts`，管理全局任务状态
- [ ] 5.2 增强 `web/src/App.tsx` 中的 WebSocket 接收逻辑，解析任务更新
- [ ] 5.3 实现页面首次加载时自动拉取活跃任务列表的同步机制
...
- [ ] 7.1 更新 `tests/CHECKLIST.md`，增加异步任务状态流转测试项
- [ ] 7.2 执行并发冲突测试：连续触发同模块任务，验证 409 返回
- [ ] 7.3 执行全链路冒烟测试：从专家克隆到网关重启
