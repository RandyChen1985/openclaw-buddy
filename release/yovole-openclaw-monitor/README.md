...
2. **故障诊断**：一旦发现服务宕机，自动对比当前的 `openclaw.json` 与备份配置的差异。
3. **自动回滚**：检测到配置错误导致的启动失败后，自动将 `openclaw.json.bak` 还原。
4. **强制自愈**：执行 `openclaw gateway --force` 强行恢复服务，并生成 Markdown 故障报表。
