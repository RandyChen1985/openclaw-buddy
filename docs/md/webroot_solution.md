...
默认前端构建使用 **Vite `base: './'`**（未设置 `VITE_WEB_ROOT` 时）：chunk 与页面 **相对路径** 加载，**无需为每个 `WEB_ROOT` 单独打包**。运行时仍由 Buddy 注入 **`window.__WEB_ROOT__`**，与 **`WEB_ROOT` 环境变量** 一致，用于 `/v1` 等 API 与 WebSocket 前缀。

**注意**：反向代理须把 **完整前缀**（含 `/assets`）转到 Buddy；访问子路径时建议使用 **尾部斜杠**（如 `/console/claw/`），避免个别浏览器对相对资源解析歧义。

若仍使用固定前缀打包，可在构建环境设置 **`VITE_WEB_ROOT=/your-prefix/`**，行为与以前一致。
...
---
*本文档由 Antigravity 自动生成，记录于 2026-04-01 重构版本；后续补充「相对 base 单构建」说明。*
