    ...
    <div style={{ height: 120 }}>
      {!isDailyUsage && (
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
          <span style={{ fontSize: 11, color: isDarkMode ? '#94a3b8' : '#64748b', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.05em' }}>{label} {t('dashboard.historyTrend')}</span>
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 20, width: '100%' }}>
      ...
      <div 
        ...
        style={{ 
          background: isDarkMode ? 'rgba(30, 41, 59, 0.7)' : 'rgba(255, 255, 255, 0.45)', 
          ...
          minHeight: 64,
          boxShadow: isDarkMode ? '0 4px 6px -1px rgba(0, 0, 0, 0.3)' : '0 4px 6px -1px rgb(0 0 0 / 0.05)',
            ...
            <div style={{ display: 'flex', alignItems: isMobile ? 'flex-start' : 'center', gap: isMobile ? '16px' : '32px', flexDirection: isMobile ? 'column' : 'row', width: isMobile ? '100%' : 'auto', flex: isMobile ? undefined : 1, minWidth: 0 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                ...
                <div>
                  ...
                  <div style={{ fontSize: 14, color: isDarkMode ? '#f1f5f9' : '#1e293b', fontWeight: 600 }}>{systemInfo?.hostname || '---'}</div>
                </div>
              </div>
              <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                <div style={{ background: isDarkMode ? '#0f172a' : '#f5f3ff', padding: '8px', borderRadius: '10px' }}><Server size={18} style={{ color: '#8b5cf6' }} /></div>
                <div>
                  <div style={{ fontSize: 10, color: '#94a3b8', textTransform: 'uppercase', letterSpacing: '0.08em', fontWeight: 600 }}>OS</div>
                  <div style={{ fontSize: 14, color: isDarkMode ? '#f1f5f9' : '#1e293b', fontWeight: 600 }}>{systemInfo ? `${systemInfo.os} (${systemInfo.arch})` : '---'}</div>
                </div>
              </div>
              {!isMobile && <div style={{ width: 1, height: 24, background: isDarkMode ? '#334155' : '#e2e8f0' }} />}
              <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                <div style={{ background: isDarkMode ? '#0f172a' : (ocStatus?.installed ? '#f0fdf4' : '#fef2f2'), padding: '8px', borderRadius: '10px' }}><Zap size={18} style={{ color: ocStatus?.installed ? '#22c55e' : '#ef4444' }} /></div>
                <div>
                  <div style={{ fontSize: 10, color: '#94a3b8', textTransform: 'uppercase', letterSpacing: '0.08em', fontWeight: 600 }}>OpenClaw CLI</div>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                    {ocStatus?.installed ? <Badge status="processing" color="#22c55e" text={<span style={{ fontSize: 14, color: isDarkMode ? '#f1f5f9' : '#1e293b', fontWeight: 700 }}>{ocStatus.version}</span>} /> : <span style={{ fontSize: 14, color: '#ef4444', fontWeight: 700 }}>未安装</span>}
                  </div>
                </div>
              </div>
              {!isMobile && <div style={{ width: 1, height: 24, background: isDarkMode ? '#334155' : '#e2e8f0' }} />}
              <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                <div style={{ background: isDarkMode ? '#0f172a' : '#ecfdf5', padding: '8px', borderRadius: '10px' }}><Activity size={18} style={{ color: '#059669' }} /></div>
                <div>
                  <div style={{ fontSize: 10, color: '#94a3b8', textTransform: 'uppercase', letterSpacing: '0.08em', fontWeight: 600, display: 'flex', alignItems: 'center', gap: 6 }}>
                    ...
                    >
                      <Mail size={11} color="#94a3b8" strokeWidth={2.5} />
                    </div>
                  </div>
                  <div style={{ fontSize: 14, color: isDarkMode ? '#f1f5f9' : '#1e293b', fontWeight: 700, display: 'flex', alignItems: 'center', gap: 6 }}>
                    v{APP_VERSION}
                    <Button 
                      type="text" 
                      size="small" 
      ...
      <Row gutter={[20, 20]}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 20, height: '100%' }}>
            <Card styles={{ body: { padding: 24 } }} style={{ borderRadius: 12, border: `1px solid ${isDarkMode ? '#334155' : '#e2e8f0'}`, background: isDarkMode ? '#1e293b' : '#fff' }}>
              ...
              <div style={{ marginBottom: 24 }}>
                ...
                <div style={{ fontSize: isMobile ? 18 : 24, fontWeight: 800, color: isDarkMode ? '#f1f5f9' : '#1e293b', fontFamily: 'monospace' }}>
                  ...
                  <>
                    <div>
                      <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 12, marginBottom: 6 }}>
                        <span style={{ color: isDarkMode ? '#94a3b8' : '#64748b' }}>{t('dashboard.cpuLoad')}</span>
                        <span style={{ fontWeight: 700, color: isDarkMode ? '#f1f5f9' : 'inherit' }}>{status?.metrics?.cpu_usage?.toFixed(1)}%</span>
                      </div>
                      <Progress percent={status?.metrics?.cpu_usage} showInfo={false} strokeColor="#3b82f6" strokeWidth={6} />
                    </div>
                    <div>
                      <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 12, marginBottom: 6 }}>
                        <span style={{ color: isDarkMode ? '#94a3b8' : '#64748b' }}>{t('dashboard.memLoad')}</span>
                        <span style={{ fontWeight: 700, color: isDarkMode ? '#f1f5f9' : 'inherit' }}>{status?.metrics?.memory_usage?.toFixed(1)}%</span>
                      </div>
              ...
              {loading ? (
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                  {topBots.map((bot, index) => (
                    <div key={bot.id} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '10px 12px', background: isDarkMode ? '#0f172a' : '#f8fafc', borderRadius: 10, border: `1px solid ${isDarkMode ? '#334155' : '#f1f5f9'}` }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                        <div style={{ width: 24, height: 24, borderRadius: 6, background: isDarkMode ? '#1e293b' : '#e2e8f0', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 10, fontWeight: 700, color: isDarkMode ? '#94a3b8' : '#64748b' }}>{index + 1}</div>
                        <div>
                          <div style={{ fontSize: 13, fontWeight: 700, color: isDarkMode ? '#f1f5f9' : '#1e293b' }}>{bot.name || bot.id}</div>
                          <div style={{ fontSize: 11, color: '#94a3b8' }}>ID: {bot.id}</div>
                        </div>
                      </div>
                      <div style={{ textAlign: 'right' }}>
                        <div style={{ fontSize: 12, fontWeight: 700, color: isDarkMode ? '#f1f5f9' : '#1e293b' }}>{bot.sessions || 0}</div>
        ...
        <Col xs={24} lg={14}>
          <Card 
            title={<span style={{ fontSize: 13, fontWeight: 600, color: isDarkMode ? '#f1f5f9' : 'inherit' }}><Activity size={15} color="#3b82f6" /> {t('dashboard.diagnosticLab')}</span>}
            styles={{ header: { borderBottom: `1px solid ${isDarkMode ? '#334155' : '#f1f5f9'}`, minHeight: 52 }, body: { padding: '24px' } }}
            style={{ height: '100%', borderRadius: 12, border: `1px solid ${isDarkMode ? '#334155' : '#e2e8f0'}`, background: isDarkMode ? '#1e293b' : '#fff' }}
          >
            <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
              ...
              <div style={{ borderTop: `1px solid ${isDarkMode ? '#334155' : '#f1f5f9'}`, paddingTop: 20 }}>
                <div style={{ display: 'flex', flexDirection: 'column', gap: 16, marginBottom: 16 }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <span style={{ fontSize: 11, color: isDarkMode ? '#94a3b8' : '#64748b', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.05em' }}>
                  ...
                  <Tabs 
                    ...
                    className={`compact-tabs usage-tabs ${isDarkMode ? 'dark-tabs' : ''}`}
                    items={[
            ...
            {/* 巡检事件轨迹 */}
            <div style={{ marginTop: 24, borderTop: `1px solid ${isDarkMode ? '#334155' : '#f1f5f9'}`, paddingTop: 24 }}>
              <div style={{ fontSize: 13, fontWeight: 600, color: isDarkMode ? '#f1f5f9' : '#475569', display: 'flex', alignItems: 'center', gap: 6, marginBottom: 20 }}>
                <Zap size={15} color={isDarkMode ? '#94a3b8' : '#64748b'} /> {t('dashboard.timeline')}
              </div>
              <div style={{ 
                height: 260, 
                overflowY: 'auto', 
                ...
                <Timeline
                  items={systemEvents.map(ev => ({
                    ...
                    children: <span style={{ color: isDarkMode ? '#cbd5e1' : 'inherit' }}>{ev.message}</span>,
                  }))}
                  style={{ paddingTop: 8 }}
                />
                {systemEvents.length === 0 && (
                  <div style={{ textAlign: 'center', color: isDarkMode ? '#64748b' : '#475569', fontSize: 12, padding: '40px 0' }}>
